function [rf_90_set_wRefocus,g_90_versed_4us,AsymmetryFac_90,rf_180_versed,g_180_versed_4us,AsymmetryFac_180,slThick,nomFlips]= PulsePrep_v4_pulseq_sinc(sinc_pulse,VerseDownFac)
...
Ts = sinc_pulse.dt*10^-3;
rf_90_set = sinc_pulse.rfEncOut; 
rf_180 = sinc_pulse.rfOtherOut; 
gAmp = sinc_pulse.gAmp/10; % in G/cm
nEncodes = size(rf_90_set,2);

gmax_90 = sinc_pulse.gAmp*1.0; %1.0;
gmax_180 = sinc_pulse.gAmp*1.0;
gmax_refoc = 2.5;
smax = 10000;  % original 12000 for siemens, use 10000 instead for GE scanners;
%% 90
g_90_noRefocus = sinc_pulse.gAmp*[ones(1,size(rf_90_set,1))];    % max gradient amplitude [G/cm]
g_Refocus = sinc_pulse.gAmp*ones(1,(round(size(rf_90_set,1)/2)));
g_90 = sinc_pulse.gAmp*[ones(1,size(rf_90_set,1)) -g_Refocus];    % max gradient amplitude [G/cm]
...
% g_90_versed = [g_90_noRefocus_versed.' -g_Refocus_versed.'];  
...
for EncodingCount = 1:nEncodes
    ...
    rf_90_versed = spline(k,rf_90,k_versed).*(g_90_noRefocus_versed/sinc_pulse.gAmp);
...
g_180 =sinc_pulse.gAmp*[ones(size(rf_180))];
