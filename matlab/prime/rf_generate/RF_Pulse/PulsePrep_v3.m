function [rf_90_set_wRefocus,g_90_versed_10us,AsymmetryFac_90,rf_180_versed,g_180_versed_10us,AsymmetryFac_180,slThick]= PulsePrep_v3(filename,VerseDownFac)
...
% rf_90_forVERSE =[floor(zeros((size(rf_90_set,1)-size(rf_180,2)+1)/2,1)); rf_180.'; ceil(zeros((size(rf_90_set,1)-size(rf_180,2)+1)/2,1))]; %QL
...
g_90_versed_wo = [g_90_noRefocus_versed.'];
...
AsymmetryFac_90 =  (length(g_90_noRefocus_versed)/2)/length(g_90_versed);
 ...
 oversample = 10^-5/Ts;
g_90_versed_10us = g_90_versed(max(floor(oversample/2),1):oversample:end); % quick hack for now
g_90_versed_no_rw_10us = g_90_versed_wo(max(floor(oversample/2),1):oversample:end); % quick hack for now

correctRFlength = length(g_90_versed_10us)*oversample;
...
g_180_versed_10us = g_180_versed(max(floor(oversample/2),1):oversample:end); % quick hack for now
correctRFlength = length(g_180_versed_10us)*oversample;
