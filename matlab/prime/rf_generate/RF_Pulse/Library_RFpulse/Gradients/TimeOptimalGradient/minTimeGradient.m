...
%               t = linspace(-1,1, 500)'; 
%               C = (5+ t.*cos(5.5*pi*t)).*exp(i*pi*t);
%               C = [real(C), imag(C), sin(5.5*pi*t)];
%               [C,time,g,s,k] = minTimeGradient(C,0,0);
%               figure, 
%               subplot(311), plot3(C(:,1), C(:,2), C(:,3),'.');
%               subplot(312), plot(g);, title('gradient');
%               subplot(313), plot(s);title('slew-rate');
%
%
%
...
s = diff(g)/dt;% (g([2:end, end]) - g([1:end-1, end-1]))/dt;
time = t(end);
