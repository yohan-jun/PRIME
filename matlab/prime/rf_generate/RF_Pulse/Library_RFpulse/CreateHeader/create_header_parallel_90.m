...
function create_header_parallel_90(g,b_est,oversample)
...
fidx = fopen('gx_excit.float', 'wb');
fidy = fopen('gy_excit.float', 'wb');
fidz = fopen('gz_excit.float', 'wb');
...
for (ii = 1:8)
    fname_rfamp = sprintf('rf_excit_amp_%d.float', ii);
    fidrf_amp = fopen(fname_rfamp, 'wb');
    fname_rfpha = sprintf('rf_excit_pha_%d.float', ii);
