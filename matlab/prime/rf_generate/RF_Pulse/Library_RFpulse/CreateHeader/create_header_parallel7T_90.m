...
function create_header_parallel7T_90(g,b_est,gap,oversample)
...
for (ii = 1:8)
    ...
    % do grads
    fidx = fopen('gx90.float', 'wb');
    fidy = fopen('gy90.float', 'wb');
    fidz = fopen('gz90.float', 'wb');
    ...
    fname_rfamp = sprintf('rf_amp90.float');
    fidrf_amp = fopen(fname_rfamp, 'wb');
    fname_rfpha = sprintf('rf_pha90.float');
