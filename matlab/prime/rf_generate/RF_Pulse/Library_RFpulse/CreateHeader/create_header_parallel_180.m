...
function create_header_parallel_180(g,b_est,oversample)
...
fidx = fopen('gx_refoc.float', 'wb');
fidy = fopen('gy_refoc.float', 'wb');
fidz = fopen('gz_refoc.float', 'wb');
...
for (ii = 1:8)
    fname_rfamp = sprintf('rf_refoc_amp_%d.float', ii);
    fidrf_amp = fopen(fname_rfamp, 'wb');
    fname_rfpha = sprintf('rf_refoc_pha_%d.float', ii);
