...
tbG = 12%14%12; % overall tb product of encoding pulse; Should be > 2*G. If G/tbG
% is too high, you may get an error about non-increasing band edges from
% firls
tbOther = 8%10%8; % tb product of non-encoding pulse
...
filename = 'RF_5x_90thick_1p03t_Verse1_B1_Mzsim_T1_1000_TR6p3'; %  
TR=3500 % ms
T1=1000 % ms
    ...
    for Gind = 1:G % sub-slice to design for
        ...
        if ~DFTphs
            if usecvx
                ...
                b = bsf*gSliderBeta_T90fac(N,G,Gind,tbG,d1,d2,phi,T90fac);
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                if doRootFlip
                    if strcmp(Gpulse,'ex')
                        phsFact = 2; % we have to square the 180 beta phase to
...
%     plot the first encoding pulse
    ...
    Mz_new_T1=E1 * Mz_new + 1 - E1;
    figure; plot(z,Mz_new(:,5));hold on;plot(z,Mz_new_T1(:,5));
    for ii=1:G
        if strcmp(Gpulse,'ex')
