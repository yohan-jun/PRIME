...
% v2 notes
%   1)  for loop for multi-slice and multi-diff-dir

% v3 notes
%   1)  reconstruct each direction separately to reduce memory requirement
% v3_v2 notes
%   1)  reconstruct pe_pf data

% v4 notes
%   1)  reconstruct image using interpolated low field map
% v4_v2 notes
%   1)  reconstruct fe_pf data

% v5_v2 notes
%   1)  reconstruct w/o nav (using first dummy scan)

% v6 notes
%   1)  SMS

% v7 notes
%   1)  handling difference freq-encoding samples across echoes
...
% 
% addpath /autofs/cluster/berkin/berkin/Matlab_Code_New/fleet_buda/mrir_toolbox
% addpath /autofs/cluster/berkin/berkin/Matlab_Code_New/LIBRARY/read_meas_dat__20140924112147
% addpath /autofs/cluster/berkin/berkin/Matlab_Code_New/LIBRARY/twixmap_philipp_ehses/
% addpath /autofs/cluster/berkin/berkin/Matlab_Code_New/fleet_buda
% 
% addpath(fullfile('/autofs/cluster/berkin/yohan/python_code/bart-0.8.00/','matlab'));
% setenv('TOOLBOX_PATH', '/autofs/cluster/berkin/yohan/python_code/bart-0.8.00/');
% 
% addpath /autofs/cluster/berkin/yohan/matlab_code/mapVBVD_LN


%% load data

file_path       = '/autofs/cluster/berkin/yohan/matlab_code/pulseq/matlab/prime/seq/032924/raw/';
file_name       = 'meas_MID00395_FID35041_prime_v14_1p1mm_E1R4_E2R4_effES_b2k_dir100_set1';
file_sens_name  = 'meas_MID00393_FID35039_gre_csm';

seq_path        = '/autofs/cluster/berkin/yohan/matlab_code/pulseq/matlab/prime/seq/032924/';
seq_name        = 'prime_v14_1p1mm_E1R4_E2R4_effES_b2k_dir100_set1.seq';
...
for ndwi = 2:3 % (index 1 : dummy scan)
% for ndwi = 2:Ndir % (index 1 : dummy scan)
    ...
    % if Ny ~= Ny_low
    %     kspace_low  = zeros([floor(Nx_low*pF_low_fe*ro_os_low),Nc,Npe_low_def*R_low*Nseg,Necho,Nseg,Ngslider,Nslices,2]);
    % else
    ...
    for dd = 1:Ndir
                ...
                for ss = 1:Nslices
                    ...
                    for ee = 1:Necho
                        if dd == 1 || dd == ndwi
                            ...
                            % kspace_tmp_     = zeros([size(kspace,1),Nc,Npe_acq]);
                            % kspace_low_tmp_ = zeros([size(kspace_low,1),Nc,Npe_acq]);
                            ...
                            for cc = 1:Nc
                                for aa = 1:Npe_acq
                                    if aa <= Npe_def
                                        ...
                                        % if R_low == 1.5
                                        %     kxmin   = min(min(sq(crop(ktraj_tmp(1,:,Npe_def+1:Npe_def+Npe_low_def),[1,Nfe*0.95,Npe_low_def]))))*(Nx_low*pF_low_fe*ro_os_low/2-1)/(Nx_low*pF_low_fe*ro_os_low/2);
                                        %     kxmax   = max(max(sq(crop(ktraj_tmp(1,:,Npe_def+1:Npe_def+Npe_low_def),[1,Nfe*0.95,Npe_low_def]))))/(Nx_low*pF_low_fe*ro_os_low/2-1)*(Nx_low*pF_low_fe*ro_os_low/2);
                                        % elseif R_low == 2
                                        %     kxmin   = min(min(sq(crop(ktraj_tmp(1,:,Npe_def+1:Npe_def+Npe_low_def),[1,Nfe*0.9,Npe_low_def]))))*(Nx_low*pF_low_fe*ro_os_low/2-1)/(Nx_low*pF_low_fe*ro_os_low/2);
                                        %     kxmax   = max(max(sq(crop(ktraj_tmp(1,:,Npe_def+1:Npe_def+Npe_low_def),[1,Nfe*0.9,Npe_low_def]))))/(Nx_low*pF_low_fe*ro_os_low/2-1)*(Nx_low*pF_low_fe*ro_os_low/2);
                                        % end
                            ...
                            if gg == 1
                                ...
                                if ee == 1
                                    ...
                                    % kspace(:,:,k_idx_1,ee,gg,ll,ss,1) = kspace_tmp_(:,:,1:2:Npe_def); % odd lines
                                    % kspace(:,:,k_idx_2,ee,gg,ll,ss,2) = flip(kspace_tmp_(:,:,2:2:Npe_def),1); % even lines
                            ...
                            elseif gg == 2
                                % if ee == 1 || (ee > 1 && Ny == Ny_low) 
                                if ee == 1
                                    % k_idx_1 = pF*Ny-R*Nseg-R*2:-R*Nseg*2:1; % mod
                                    % k_idx_2 = pF*Ny-R*2:-R*Nseg*2:1; % mod
                                    k_idx_1 = pF*Ny-R*Nseg-round(R):-R*Nseg*2:1; % 1mm (R5)
                                    k_idx_2 = pF*Ny-round(R):-R*Nseg*2:1; % 1mm (R5)
                                    % k_idx_1 = pF*Ny-R*Nseg:-R*Nseg*2:1; % mod 3
                                    % k_idx_2 = pF*Ny:-R*Nseg*2:1; % mod 3
                                    kspace(:,:,k_idx_2,ee,gg,ll,ss,1) = kspace_tmp_(:,:,1:2:Npe_def); % odd lines for 1mm/780u
                                    kspace(:,:,k_idx_1,ee,gg,ll,ss,2) = flip(kspace_tmp_(:,:,2:2:Npe_def-1),1); % even lines for 1mm/780u
                                    % kspace(:,:,k_idx_2,ee,gg,ll,ss,1) = kspace_tmp_(:,:,1:2:Npe_def); % odd lines for 780u
                                    % kspace(:,:,k_idx_1,ee,gg,ll,ss,2) = flip(kspace_tmp_(:,:,2:2:Npe_def),1); % even lines for 780u
                                else
                                    ...
                                    kspace_low(:,:,k_idx_2,ee,gg,ll,ss,1) = kspace_low_tmp_(:,:,Npe_def+1:2:Npe_def+Npe_low_def); % odd lines for 1mm
                                    kspace_low(:,:,k_idx_1,ee,gg,ll,ss,2) = flip(kspace_low_tmp_(:,:,Npe_def+2:2:Npe_def+Npe_low_def-1),1); % even lines for 1mm
                                    % kspace_low(:,:,k_idx_2(1:length(Npe_def+1:2:Npe_def+Npe_low_def-1)),ee,gg,ll,ss,1) = kspace_low_tmp_(:,:,Npe_def+1:2:Npe_def+Npe_low_def-1); % odd lines for 780u
                                    % kspace_low(:,:,k_idx_1(1:length(Npe_def+2:2:Npe_def+Npe_low_def-1)),ee,gg,ll,ss,2) = flip(kspace_low_tmp_(:,:,Npe_def+2:2:Npe_def+Npe_low_def-1),1); % even lines for 780u
                                end
                            end
                        end
                    end
        ...
        if dd == 1
            %             kspace_nav_ = kspace(:,:,end-2:end,:,:,:,:,:);
            %             kspace_low_nav_ = kspace_low(:,:,end-2:end,:,:,:,:,:);
    ...
    % if Ny ~= Ny_low
    %     kspace_low_ = zeros([Nx_low*pF_low_fe,Npe_low_def*R_low*Nseg,Nc,1,Necho,1,Nseg,2,Ngslider,Nslices]);
    % else
    ...
    for rr = 1:Nseg % AP/PA
        ...
        if rr == 1
            ...
            % tmp correction
            % kspace_low_cor_full_ap  = sq(padarray(kspace_low_cor_deblur,[0,pad_factor_low],'pre'));   % for ap direction
            ...
            % tmp correction
            % kspace_low_cor_full_pa  = sq(padarray(kspace_low_cor_deblur,[0,pad_factor_low],'post'));  % for pa direction
    ...
    if AccY == 4
        % del_ky_shift    = [2,-1]; % org R4 110/108 % use this
        % del_ky_shift    = [2,3]; % org R4 110/108 % use this
        del_ky_shift    = [-1,2]; % org R4 100/98 % use this
        % del_ky_shift    = [-1,2]; % org R4 140/138
    elseif AccY == 5
        del_ky_shift        = [1,-1]; % org R5 110/108
        % del_ky_shift        = [1,-1]; % org R5 770u 140/138
        % del_ky_shift        = [1,-3]; % org R5 610u 180/178
    end

    if AccY_low == 2
        del_ky_low_shift    = [1,-1]; % org R=2 (Ny_low=72, 3.05mm2)
    elseif AccY_low == 3
        %         del_ky_low_shift    = [-1,-2]; % org 56/55 R=3 (Ny_low=110, 2mm2)
        del_ky_low_shift    = [0,0]; % org 55/53 R=3 (Ny_low=110, 2mm2)
    elseif AccY_low == 4
        % del_ky_low_shift    = [3,-1]; % org 110/108 R=4 (Ny_low=220, 1mm2) % use this
        del_ky_low_shift    = [1,0]; % org 100/98 R=4 (Ny_low=200, 1.1mm2)
        % del_ky_low_shift    = [1,0]; % org 55/53 R=4 (Ny_low=280, 780u)
    end
    ...
    if AccY == 4
        ...
        % del_ky_a    = [1,3]; % org R4 110/108
        % del_ky_a    = [3,1]; % org R4 140/138
    elseif AccY == 5
        del_ky_a    = [4,2]; % org R5 110/108
        % del_ky_a    = [4,2]; % org R5 770u 140/138
        % del_ky_a    = [4,2]; % org R5 610u 180/178
    end
    ...
    Ry_low          = AccY_low;
    %         del_ky_low_a    = [1,0]; % org R=2 (Ny_low=72, 3.05mm2)
    %         del_ky_low_a    = [1,0]; % org R=3 (Ny_low=110, 2mm2)
    del_ky_low_a    = [3,1]; % org R=4 (Ny_low=200, 1.1mm2)
    % del_ky_low_a    = [0,1]; % org R=3 (Ny_low=110, 2mm2)
    % del_ky_low_a    = [1,3]; % org R=4 (Ny_low=220, 1mm2)
    % del_ky_low_a    = [3,1]; % org R=4 (Ny_low=220, 780u)
    ...
    %     img_low_sense_pad   = ifft2call(padarray(fft2call(apodize(img_low_sense,1.0)),(size(img_sense)-size(img_low_sense))/2)); % v4
    ...
    % for gs_index = 1:1
        ...
        % addpath /cluster/berkin/berkin/Matlab_Code_New/BUDA_noddi
        config_path = '/autofs/cluster/berkin/yohan/matlab_code/pulseq/matlab/prime/recon/';
