...
%     ipeak=find(abs(rf.signal)>=rfmax*0.99999);
  if size(rf.signal,1)>1
      if mod(size(rf.signal,1),2)==0
    tc=rf.t((0.5*size(rf.signal,1)));
      else
           tc=rf.t((0.5*(size(rf.signal,1)+1)));
      end
  else
       % tc=rf.t((0.5*size(rf.signal,2)));
       tc=rf.t(round(0.5*size(rf.signal,2))); % YJ
  end
