classdef twix_map_obj_MIN < handle
...
methods
    % Constructor:
    function this = twix_map_obj_MIN(arg,dataType,fname,version,rstraj)
...
%         end
    ...
    function this = clean(this)
        ...
        % the same for all mdhs: % Comment: All NCol are not the same in ESISTINA sequence!!!. Qingping CHEN, 13.07.2020
        % this.NCol = this.NCol(1); % Comment by Qingping CHEN, 13.07.2020
        this.NCol = min(this.NCol);
    ...
    function out = readData(this,mem,cIxToTarg,cIxToRaw,selRange,selRangeSz,outSize)
        ...
        for k = 1:kMax
            ...
            if blockCtr == blockSz || k == kMax || (isBrokenRead && blockCtr > 1)
                ...
                % reflect fids when necessary S.sHIN
